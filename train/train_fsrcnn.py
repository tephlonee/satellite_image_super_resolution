...
from evaluation.losses import build_fsrcnn_criterion
...
class FSRCNNTrainer:
    def __init__(self, cfg, train_loader, val_loader, resume: Optional[str] = None):
        self.cfg = cfg
        self.train_cfg = cfg.train_fsrcnn
        self.device = get_device(cfg.get("device", "auto"))

        self.model = build_fsrcnn(cfg).to(self.device)
        logger.info(f"FSRCNN params: {self.model.n_parameters():,}")

        self.criterion = build_fsrcnn_criterion(cfg)
        ...
        log_dir = Path(self.train_cfg.get("checkpoint_dir", "checkpoints/fsrcnn"))
    ...
    def train(self) -> dict:
        ...
        ckpt_dir = self.train_cfg.get("checkpoint_dir", "checkpoints/fsrcnn")

        logger.info(f"Starting FSRCNN training for {total_epochs} epochs")
