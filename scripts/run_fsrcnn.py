...
from train.train_fsrcnn import FSRCNNTrainer

logger = get_logger(__name__)


def parse_args():
    parser = argparse.ArgumentParser(description="Train FSRCNN for SAR Super-Resolution")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("SAR Super-Resolution | FSRCNN Training")
    logger.info("=" * 60)
    ...
    train_loader, val_loader, test_loader, preprocessor = build_dataloaders(
        cfg,
        batch_size=cfg.train_fsrcnn.batch_size,
    )
    ...
    best_ckpt = f"{cfg.train_fsrcnn.checkpoint_dir}/best_model.pth"
    ...
    save_train_info(results, cfg.train_fsrcnn.train_log_dir, model_name="fsrcnn")
